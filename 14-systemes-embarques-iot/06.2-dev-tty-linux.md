...
**Méthode 2 : Commande dmesg (messages kernel)**
```bash
...
**Solution 2 : Changer permissions temporairement (non recommandé)**
```bash
...
**c_iflag (Input flags)** - Traitement entrée :
...
**Après ouverture, désactiver mode non-bloquant si nécessaire :**
...
begin
  ...
  try
    parties.Delimiter := ',';
    parties.StrictDelimiter := True;
    parties.DelimitedText := trame;

    if parties.Count < 15 then
      Exit;
...
### Exemple 3 : Contrôle Arduino avec protocole
...
**2. Utiliser un thread de lecture dédié :**
...
### Annexe E : Checklist de dépannage
...
**Programme complet : Echo série simple**
...
**Utilisation (même code sur Windows et Linux) :**
...
# Puis écrivez un programme minimal
```

**2. Vérifiez toujours les permissions :**
```bash
sudo usermod -a -G dialout $USER
...
```bash
dmesg | grep tty    # Voir détection
lsusb               # Voir périphériques USB
...
begin
  case fpgeterrno of
    ESysEACCES: ShowMessage('Permission refusée');
    ESysENOENT: ShowMessage('Périphérique inexistant');
    ESysEBUSY: ShowMessage('Port occupé');
  end;
  Exit;
end;
```
...
```pascal
// Thread dédié évite blocage interface
TThreadLecture = class(TThread)
  // ... lecture en arrière-plan
end;
```

**3. Implémenter reconnexion automatique :**
```pascal
...
**Pages man Linux :**
```bash
man termios     # Configuration port série
man tcsetattr   # Fonctions configuration
man select      # Multiplexage I/O
man ioctl_tty   # Contrôle terminal
```
...
- Exemples de code réels et testés

### Projets open source utilisant série

**Exemples à étudier :**
- PascalSCADA: Framework industriel avec communication série
- mORMot: Framework avec support série
- Divers projets Arduino + FreePascal

## Exercice final : Projet complet

Pour valider votre apprentissage, créez un **moniteur de température multi-capteurs** :

**Spécifications :**
- Lire 3 capteurs DS18B20 via Arduino
- Interface Lazarus avec graphique temps réel
...
```
[Arduino] ←série→ [Application FreePascal]
                       ↓
                  ┌────┴────┐
                  │         │
            [GUI Lazarus] [Thread]
...
Vous maîtrisez maintenant la **communication série multi-plateforme** avec FreePascal :

**Compétences acquises :**
- ✅ Comprendre architecture ports série Windows et Linux
- ✅ Configurer et ouvrir ports série
- ✅ Lire et écrire données (texte et binaire)
- ✅ Gérer timeouts et flow control
- ✅ Contrôler signaux matériels (DTR, RTS)
- ✅ Énumérer ports disponibles
- ✅ Déboguer problèmes courants
- ✅ Optimiser performances
- ✅ Créer applications GUI avec Lazarus
- ✅ Écrire code portable Windows/Linux

**Applications possibles :**
- 🤖 Communication avec Arduino/STM32
- 📡 Lecture GPS et télémétrie
- 🏭 Protocoles industriels (ModBus RTU)
- 🔬 Instrumentation scientifique
- 📊 Acquisition de données multi-capteurs
- 🎮 Interfaces de contrôle personnalisées

**Prochaines étapes suggérées :**
1. Implémenter un protocole binaire personnalisé
2. Créer un analyseur de protocole (type Wireshark pour série)
3. Intégrer avec base de données pour logging long terme
4. Développer interface web (serveur série → WebSocket)
5. Explorer RS-485 pour réseaux multi-points

**N'oubliez pas :**
- Toujours vérifier permissions sur Linux
- Gérer proprement les erreurs et reconnexions
- Documenter votre protocole de communication
- Tester sur vraie matériel, pas seulement en simulation
- Contribuer à la communauté (partager vos bibliothèques)

**Bon développement série avec FreePascal ! 🚀**

---

*Ce tutoriel fait partie du module 14.6 "Communication série" de la formation "FreePascal/Lazarus - Niveau Développeur Avancé - Edition Multi-plateforme Windows/Ubuntu"*

*Section précédente : 14.6.1 Ports COM Windows*

*Prochaine section : 14.7 GPIO et interfaces matérielles*

⏭️ [GPIO et interfaces matérielles](/14-systemes-embarques-iot/07-gpio-interfaces-materielles.md)
