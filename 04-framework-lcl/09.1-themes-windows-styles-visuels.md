...
begin
  ...
  // Afficher des informations sur le thème actuel
  Caption := Format('Thème actif - DPI : %d', [Screen.PixelsPerInch]);
end;
```
...
Windows fournit l'API UxTheme pour dessiner des éléments avec le style visuel actuel. Lazarus encapsule cette API dans `ThemeServices`.
...
### Dessiner des contrôles personnalisés avec le thème
...
Windows envoie des messages quand le thème change :
...
## Intégration avec les fonctionnalités Windows modernes
...
begin
  // Dans Lazarus, les styles visuels sont contrôlés par le manifeste
  // à la compilation (pas de propriété UseVisualStyles comme en Delphi).
  // Pour tester sans thèmes, vérifier ThemesEnabled au runtime :
  if ThemeServices.ThemesEnabled then
    WriteLn('Thèmes actifs : tester le rendu thématique')
  else
    WriteLn('Thèmes inactifs : tester le rendu classique');
end;
```
