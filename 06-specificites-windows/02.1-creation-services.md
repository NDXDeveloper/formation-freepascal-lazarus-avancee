...
# Création de Services Windows avec FreePascal/Lazarus

## Introduction : Votre premier service Windows

Créer un service Windows peut sembler intimidant au début, mais FreePascal/Lazarus simplifie grandement le processus grâce à son framework de services intégré. Dans ce tutoriel, nous allons créer pas à pas différents types de services, du plus simple au plus complexe.
...
1. **Lazarus installé** avec les packages standards
2. **Droits administrateur** sur votre machine Windows (nécessaire pour installer des services)
3. **Le package daemonapp** (normalement inclus dans Lazarus)

Pour vérifier la présence du package :
- Ouvrez Lazarus
- Menu `Package` → `Ouvrir un package`
- Cherchez `daemonapp` dans la liste

### Concepts de base FreePascal pour les services

FreePascal utilise plusieurs classes pour créer des services :

- **TCustomDaemon** : La classe de base pour créer un service
- **TCustomDaemonApplication** : L'application qui héberge le service
- **TDaemonMapper** : Fait le lien entre le service et l'application

## Méthode 1 : Utiliser l'assistant Lazarus (Recommandé pour débuter)

### Étape 1 : Créer un nouveau projet de service

1. Dans Lazarus : `Fichier` → `Nouveau...`
2. Dans la fenêtre qui s'ouvre, choisissez `Projet`
3. Sélectionnez `Application Daemon (Service)`
4. Cliquez sur `OK`

Lazarus génère automatiquement la structure de base :
- Un fichier projet (.lpr)
- Une unité pour le daemon
- Un mapper pour lier le tout

### Étape 2 : Comprendre la structure générée

Voici ce que Lazarus a créé pour vous :
...
begin
  ...
  // Crée et enregistre le mapper
  Application.Run;
end.
```

**Unité du daemon (Unit1.pas) :**
```pascal
unit Unit1;
...
begin
  ...
  // Mettre OK à False empêche le démarrage
  OK := True;
end;
...
### Étape 3 : Les événements principaux du service

Chaque service a des événements clés que vous devez comprendre :

#### OnStart (DataModuleStart)
```pascal
procedure TDaemon1.DataModuleStart(Sender: TCustomDaemon; var OK: Boolean);  
begin
  // Appelé quand le service démarre
  // Utilisé pour :
  // - Initialiser les variables
  // - Ouvrir les connexions (base de données, réseau)
  ...
  try
    ...
    OK := True; // Le service peut démarrer
  except
    on E: Exception do
    begin
      LogError('Erreur au démarrage : ' + E.Message);
      OK := False; // Empêche le démarrage du service
    end;
  end;
end;
```
...
procedure TDaemon1.DataModuleStop(Sender: TCustomDaemon; var OK: Boolean);  
begin
  // Appelé quand le service s'arrête
  // Utilisé pour :
  // - Sauvegarder l'état
  ...
  try
    ...
    on E: Exception do
    begin
      LogError('Erreur à l''arrêt : ' + E.Message);
      OK := True; // On arrête quand même
    end;
  end;
end;
```

## Méthode 2 : Création manuelle (Pour mieux comprendre)

Si vous voulez comprendre ce qui se passe sous le capot, créons un service manuellement :

### Structure complète d'un service manuel
...
var
  LogFileName: string;
begin
  // Créer un fichier log dans le dossier du service
...
begin
  ...
  try
    InitializeLog;
    WriteLog('Démarrage du service...');

    // Initialisation de vos composants
    // Par exemple : connexion base de données, ouverture de ports, etc.

    WriteLog('Service démarré avec succès');
  except
    on E: Exception do
    begin
...
begin
  ...
  try
    ...
    on E: Exception do
    begin
      // En cas d'erreur, on essaie quand même de fermer le log
      try
        WriteLog('Erreur lors de l''arrêt : ' + E.Message);
        FinalizeLog;
      except
...
begin
  ...
  begin
    try
      ...
      // Exemple de tâche : écrire dans le log toutes les 10 secondes
      if (Counter mod 10) = 0 then
      begin
        WriteLog('Service en cours d''exécution (compteur: ' + IntToStr(Counter) + ')');

        // Ici, vous mettriez votre vraie logique métier
        // Par exemple : vérifier des fichiers, traiter des données, etc.
      end;
...
Un service DOIT avoir un système de logs robuste :
...
begin
  ...
  FLogLevel := llInfo; // Niveau par défaut
end;
...
begin
  if Level < FLogLevel then
    Exit; // Ne pas logger si en dessous du niveau configuré
...
### 3. Gestion des threads pour les tâches longues

Les services utilisent souvent des threads pour les tâches :
...
begin
  Terminate;
  FTerminateEvent.SetEvent; // Réveiller le thread s'il est en attente
end;

procedure TWorkerThread.Execute;  
begin
  FLogger.Log(llInfo, 'Thread de travail démarré');
  ...
  begin
    try
      ...
      begin
        ...
        Sleep(1000); // Pause en cas d'erreur pour éviter les boucles infinies
      end;
    end;
  end;

  FLogger.Log(llInfo, 'Thread de travail arrêté');
end;
...
begin
  ...
  // Exemple :
  FLogger.Log(llDebug, 'Exécution du travail...');
end;
```
...
begin
  ...
  try
    ...
    FLogger.Log(llInfo, 'Service de surveillance de dossier démarré');
...
begin
  ...
  try
    FLogger.Log(llInfo, 'Arrêt du service de surveillance');
...
begin
  ...
  begin
    try
      repeat
        // Ignorer les dossiers et fichiers spéciaux
        ...
        begin
          FLogger.Log(llInfo, Format('Nouveau fichier détecté : %s', [SearchRec.Name]));
...
begin
  try
    ...
    // Si tout va bien, déplacer vers Processed
    MoveFile(FileName, FProcessedFolder);
    FLogger.Log(llInfo, 'Fichier traité avec succès');

  except
    on E: Exception do
    begin
      FLogger.LogException(E, Format('ProcessFile(%s)', [FileName]));
      // En cas d'erreur, déplacer vers Error
      try
        MoveFile(FileName, FErrorFolder);
      except
        // Si on ne peut pas déplacer, au moins logger
        FLogger.Log(llError, 'Impossible de déplacer le fichier en erreur');
      end;
    end;
  end;
end;
...
begin
  ...
  if not RenameFile(Source, DestFile) then
    raise Exception.CreateFmt('Impossible de déplacer %s vers %s',
      [Source, DestFile]);
end;
...
Votre service compilé peut être installé de plusieurs façons :

```batch
REM Installation simple  
MonService.exe /install

REM Installation avec démarrage automatique  
MonService.exe /install /auto

REM Désinstallation  
MonService.exe /uninstall

REM Exécution en mode console (pour debug)  
MonService.exe /run
```

### Code pour gérer les paramètres de ligne de commande
...
begin
  ...
  begin
    ...
    begin
      ...
      WriteLn('Exécution en mode console...');
      WriteLn('Appuyez sur Ctrl+C pour arrêter');

      // Créer et exécuter le service manuellement
    ...
    begin
      ...
      WriteLn('  /uninstall  : Désinstaller le service');
      WriteLn('  /run        : Exécuter en mode console (debug)');
      WriteLn('  /help       : Afficher cette aide');
      Halt(0);
    end;
  end;

  // Si pas de paramètre spécial, laisser DaemonApp gérer
end;
...
### Script PowerShell pour gérer le service
...
switch ($Action) {
    "install" {
        ...
        Write-Host "Service installé et configuré" -ForegroundColor Green
    }
    ...
    "uninstall" {
        Write-Host "Désinstallation du service..." -ForegroundColor Red
...
Ajoutez un mode debug à votre service :
...
### Attachement au service en cours d'exécution

Pour déboguer un service déjà démarré :

1. Compilez avec les informations de debug
2. Démarrez le service normalement
3. Dans Lazarus : `Run` → `Attach to Process`
...
begin
  // Réinitialiser le compteur si la dernière erreur date de plus d'1 heure
  if (FLastError > 0) and (Now - FLastError > OneHour) then
  begin
    FRetryCount := 0;
    FLogger.Log(llInfo, 'Compteur de tentatives réinitialisé (dernière erreur > 1h)');
  end;
end;
...
begin
  ...
  begin
    FLogger.Log(llInfo, Format('Opération réussie après %d tentative(s)', [FRetryCount]));
    FRetryCount := 0;
    FLastError := 0;
  end;
end;
...
begin
  ...
  FLogger.Log(llWarning, Format('Tentative %d/%d échouée : %s',
  ...
  try
    repeat
      try
        ...
        // Si réussi, enregistrer le succès
        RetryManager.RecordSuccess;
        FLogger.Log(llInfo, 'Connexion à la base de données établie');
        ...
        begin
          ...
          if RetryManager.ShouldRetry then
            ...
            raise; // Propager l'exception après toutes les tentatives
        end;
      end;
    until False;
  finally
...
## Surveillance de l'état du service (Health Monitoring)

### Système de santé du service
...
begin
  FChecks.Add(Check);
  FLogger.Log(llInfo, Format('Health check enregistré : %s', [Check.Name]));
end;
...
begin
  ...
  begin
    ...
    begin
      // Ici, vous appelleriez la méthode de vérification spécifique
      // Pour l'exemple, on simule
      FLogger.Log(llDebug, Format('Exécution du check : %s', [Check.Name]));
    end;
  end;
...
var
  ...
  StatusStr: string;
begin
  FLogger.Log(llInfo, '=== Rapport de santé du service ===');
  ...
  FLogger.Log(llInfo, Format('État global : %s', [StatusStr]));
...
begin
  inherited Create('Base de données', 60); // Check toutes les 60 secondes
  FDatabase := Database;
end;
...
begin
  ...
  begin
    ...
    UpdateStatus(hsCritical, 'Impossible de vérifier l''espace disque');
end;
...
begin
  inherited Create('Utilisation mémoire', 30); // Check toutes les 30 secondes
...
begin
  ...
  if UsedMB > FMaxMemoryMB then
    UpdateStatus(hsCritical, Format('%d MB utilisés (max: %d MB)',
      [UsedMB, FMaxMemoryMB]))
  else if UsedMB > (FMaxMemoryMB * 80 div 100) then
    UpdateStatus(hsWarning, Format('%d MB utilisés', [UsedMB]))
  else
    UpdateStatus(hsHealthy, Format('%d MB utilisés', [UsedMB]));
end;
```
...
begin
  ...
  try
    ...
    FLogger.Log(llInfo, Format('Serveur HTTP démarré sur le port %d', [FPort]));
    FServer.Active := True;
    ...
    FServer.Free;
    FLogger.Log(llInfo, 'Serveur HTTP arrêté');
  end;
end;
  ...
  try
    ...
    begin
      ...
      try
        ...
        // Ajouter les détails des checks
    ...
    begin
      // Endpoint de métriques (format Prometheus)
      AResponseInfo.ContentType := 'text/plain';
...
begin
  FLogger.Log(llInfo, Format('Démarrage du serveur Named Pipe : %s', [FPipeName]));
  ...
  begin
    ...
    begin
      FLogger.Log(llError, 'Impossible de créer le Named Pipe');
      Break;
    end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          FLogger.Log(llDebug, Format('Commande reçue : %s', [Command]));
  ...
  FLogger.Log(llInfo, 'Arrêt du serveur Named Pipe');
end;
...
begin
  ...
  begin
    ...
    FLogger.Log(llInfo, 'Configuration rechargée via Named Pipe');
  end
  else if Cmd = 'PROCESS' then
  begin
    // Traiter un fichier spécifique
    Response := Format('PROCESSING %s', [Param]);
    FLogger.Log(llInfo, Format('Traitement forcé de : %s', [Param]));
  end
  else
...
begin
  ...
  begin
    ...
    WriteLn('Assurez-vous que le service est démarré');
    Exit;
  end;
...
var
  Command, Response: string;
begin
  WriteLn('Client de contrôle du service');
  ...
  repeat
    ...
    if Response <> '' then
      ...
      WriteLn('Pas de réponse du service');
...
begin
  ...
  FLogger.Log(llInfo, Format('Pool de %d threads créé', [MaxThreads]));
end;
...
begin
  ...
  FLogger.Log(llDebug, 'Travail ajouté à la file');
end;
...
begin
  ...
  FThreads.Clear;
  FLogger.Log(llInfo, 'Pool de threads arrêté');
end;
...
begin
  ...
  begin
    // Récupérer un travail de la file
...
MonService/
├── src/
│   ├── MonService.lpr          # Point d'entrée principal
│   ├── ServiceUnit.pas         # Logique du service
│   ├── ConfigManager.pas       # Gestion de la configuration
│   ├── Logger.pas              # Système de journalisation
│   ├── HealthMonitor.pas       # Surveillance de santé
│   └── BusinessLogic.pas       # Logique métier
├── config/
│   ├── service.ini             # Configuration par défaut
│   └── service.ini.example     # Exemple de configuration
├── scripts/
...
begin
  ...
  FLogger.Log(llInfo, '=== Démarrage du service ===');
  ...
  begin
    ...
    FHttpServer.Start;
    FLogger.Log(llInfo, Format('Serveur HTTP démarré sur le port %d',
      [FConfig.HttpPort]));
  end;

  // Démarrer le serveur Named Pipe si configuré
  if FConfig.EnableNamedPipe then
  begin
    FPipeServer := TNamedPipeServer.Create(
      FConfig.PipeName, FLogger);
    FPipeServer.Start;
    FLogger.Log(llInfo, Format('Named Pipe créé : %s', [FConfig.PipeName]));
  end;
...
begin
  FLogger.Log(llInfo, 'Arrêt des composants...');
  ...
  // Écrire les statistiques finales
  FLogger.Log(llInfo, Format('Statistiques finales : %d traités, %d erreurs',
    [FProcessedCount, FErrorCount]));
  FLogger.Log(llInfo, Format('Durée d''exécution : %s', [GetUptime]));
  ...
  FLogger.Log(llInfo, '=== Service arrêté ===');
  FLogger.Free;
end;
...
begin
  ...
  // Logger les paramètres importants
...
begin
  // Enregistrer les vérifications de santé
  ...
  FLogger.Log(llInfo, 'Vérifications de santé configurées');
end;
...
begin
  ...
  try
    ...
    FLogger.Log(llInfo, 'Service démarré avec succès');
  except
    on E: Exception do
    begin
      if Assigned(FLogger) then
        ...
        EventLog(etError, 'Erreur au démarrage : ' + E.Message);
...
begin
  ...
  try
    FLogger.Log(llInfo, 'Arrêt du service demandé');
    FinalizeComponents;
  except
    on E: Exception do
    begin
      if Assigned(FLogger) then
        FLogger.LogException(E, 'Stop');
      // On retourne True quand même pour permettre l'arrêt
    end;
  end;
end;
...
begin
  Result := True;
  CheckInterval := 60; // Vérification de santé toutes les 60 secondes
  LastHealthCheck := 0;
  ...
  begin
    try
      ...
      begin
        ...
        // Logger un résumé toutes les heures
  ...
  FLogger.Log(llInfo, 'Boucle principale terminée');
end;
...
begin
  ...
  begin
    ...
    EventLog(etInfo, 'Service installé avec succès');
...
begin
  // Nettoyer avant la désinstallation
  EventLog(etInfo, 'Désinstallation du service');
...
begin
  ...
  try
    ...
    begin
      WriteLn('Service démarré en mode console');
...
begin
  ...
  WriteLn('  /install       Installer le service');
  WriteLn('  /uninstall     Désinstaller le service');
  WriteLn('  /console       Exécuter en mode console (debug)');
  WriteLn('  /help          Afficher cette aide');
  WriteLn;
  WriteLn('Sans paramètre, le programme s''exécute comme service Windows');
end;
...
begin
  ...
  // Créer et configurer le mapper
  with TMyServiceMapper.Create(nil) do
  begin
    DaemonDef.Name := 'MyCompleteService';
    DaemonDef.DisplayName := 'Mon Service Complet FreePascal';
    DaemonDef.Description := 'Service Windows complet avec monitoring et gestion avancée';
    DaemonDef.ServiceType := stWin32;
    DaemonDef.StartType := stAuto; // Démarrage automatique
    DaemonDef.ErrorSeverity := esNormal;
    DaemonDef.DaemonClass := TMyService;

    // Options de récupération en cas de crash
    DaemonDef.WinBindings.Dependencies.Add('EventLog'); // Dépendance
...
## Scripts de déploiement et installation
...
:: Vérifier l'existence du fichier exe
...
:: Définir la récupération en cas d'échec
sc failure %SERVICE_NAME% reset= 86400 actions= restart/60000/restart/60000/restart/60000 >nul 2>&1

:: Définir la description
sc description %SERVICE_NAME% "Service Windows complet avec monitoring et gestion avancée" >nul 2>&1

:: Définir les dépendances (exemple : EventLog)
...
if %errorlevel% equ 0 (
    ...
    echo.
    echo ATTENTION : Le service est installé mais n'a pas pu démarrer.
...
### Script PowerShell avancé (manage.ps1)
...
.DESCRIPTION
    Permet d'installer, configurer, surveiller et gérer le service
.PARAMETER Action
    Action à effectuer : install, uninstall, start, stop, restart, status, logs, config
#>
...
function Install-Service {
    ...
    @($LogPath, (Split-Path $ConfigPath), $BackupPath) | ForEach-Object {
        if (-not (Test-Path $_)) {
            ...
            Write-Host "Répertoire créé : $_" -ForegroundColor Gray
        }
    }
...
# Fonction pour désinstaller le service
function Uninstall-Service {
    Write-Host "Désinstallation du service $ServiceName..." -ForegroundColor Yellow
    ...
    if ($service) {
        if ($service.Status -eq 'Running') {
            ...
            Write-Host "Service arrêté" -ForegroundColor Gray
        }

        & $ServicePath /uninstall
        Write-Host "Service désinstallé" -ForegroundColor Yellow
    } else {
...
function Show-Status {
    ...
    if ($service) {
        ...
        Write-Host "Nom affiché : $($service.DisplayName)"
        ...
        Write-Host "Type démar. : $($service.StartType)"

        # Informations supplémentaires via WMI
        ...
        if ($service.Status -eq 'Running' -and $wmiService.ProcessId) {
            ...
            if ($process) {
                ...
                Write-Host "Mémoire     : $([math]::Round($process.WorkingSet / 1MB, 2)) MB"
                Write-Host "Threads     : $($process.Threads.Count)"
                Write-Host "Handles     : $($process.HandleCount)"
                Write-Host "Démarré     : $($process.StartTime)"
...
function Check-Health {
    Write-Host "`n=== Vérification de santé ===" -ForegroundColor Cyan
...
function Backup-Service {
    ...
    @(
        ...
        if (Test-Path $_.Source) {
            ...
            Write-Host "  Sauvegardé : $($_.Dest)" -ForegroundColor Gray
        }
    }
    ...
    Write-Host "Sauvegarde créée : $zipFile" -ForegroundColor Green
}
...
- [ ] **Tests complets**
  ...
  - Tests de récupération d'erreur validés

- [ ] **Documentation**
  - README.md à jour
  - Guide d'installation rédigé
  - Guide de configuration documenté
  - Procédures de dépannage écrites
  - Changelog mis à jour

- [ ] **Configuration**
  - Fichier de configuration par défaut créé
  - Variables d'environnement documentées
  - Chemins relatifs utilisés (pas de chemins absolus)
  - Paramètres sensibles externalisés

- [ ] **Sécurité**
  - Compte de service avec privilèges minimaux
  - Mots de passe chiffrés dans la configuration
  - Communications sécurisées (HTTPS/TLS si applicable)
  - Validation des entrées implémentée
  - Pas d'informations sensibles dans les logs

- [ ] **Performance**
  - Fuites mémoire vérifiées
  - Gestion des ressources optimisée
  - Limites de threads configurées
  - Timeouts appropriés définis

- [ ] **Logging**
  - Rotation des logs configurée
  - Niveaux de log appropriés
  - Informations suffisantes pour le débogage
...
- [ ] **Préparation de l'environnement**
  ...
  - Prérequis installés (.NET Framework si nécessaire)
  - Antivirus configuré pour exclure les dossiers du service
  - Pare-feu configuré si nécessaire

- [ ] **Installation**
  - Service installé avec le bon compte
...
- [ ] **Vérification post-installation**
  ...
  - Logs créés au bon endroit
  - Connexions établies (base de données, réseau)
...
- [ ] **Documentation finale**
  ...
  - Historique des déploiements maintenu
...
**Symptôme** : Erreur 1053 "Le service n'a pas répondu à la demande de démarrage"

**Causes possibles et solutions** :

```pascal
// PROBLÈME : Initialisation trop longue
// MAUVAIS
function TMyService.Start: Boolean;  
begin
  // Traitement long de 30 secondes
  LoadHugeDatabase;  // Windows timeout après 30 secondes
  Result := True;
end;

// BON : démarrer un thread pour le traitement long
// Note : en mode ObjFPC, les procédures anonymes nécessitent
...
begin
  ...
  // Démarrer un thread pour le traitement long
...
```batch
@echo off
echo Diagnostic du service qui ne démarre pas  
echo ========================================

:: Vérifier l'Event Log
echo.  
echo Dernières erreurs dans l'Event Log :  
...
#### 2. Le service s'arrête immédiatement

**Symptôme** : Le service démarre puis s'arrête aussitôt

**Vérification du problème** :
```pascal
// Ajouter des logs détaillés au démarrage
function TMyService.Start: Boolean;  
begin
  Result := True;

  try
    EventLog(etInfo, 'Start: Début');
    ...
    // Test 3 : Permissions
    EventLog(etInfo, 'Start: Vérification permissions');
    CheckPermissions;

    EventLog(etInfo, 'Start: Succès');
...
begin
  ...
  begin
    FLogger.Log(llWarning, Format(
      'Augmentation mémoire significative : %.1f%% (Initial: %d MB, Actuel: %d MB)',
...
#### 4. Le service ne répond plus (freeze)
...
begin
  ...
  begin
    ...
    begin
      FLogger.Log(llCritical, 'WATCHDOG: Timeout détecté!');

      // Générer un dump pour analyse
      CreateMiniDump;

      // Notifier ou redémarrer
...
### Intégration avec Windows Performance Monitor

> **Note** : FreePascal n'intègre pas de wrapper natif pour les compteurs de performance Windows. L'approche recommandée est d'exposer vos métriques via un endpoint HTTP (comme `/metrics` au format Prometheus) ou d'écrire dans l'Observateur d'événements Windows.
...
Chargement...
    ...
    <script>
        function updateStatus() {
            ...
            // Récupérer les vérifications de santé
...
begin
  ...
  FLogger.Log('Début de la mise à jour du service ' + FServiceName);
  ...
  try
    ...
    begin
      FLogger.Log('Version déjà à jour ou plus récente');
      Exit(True);
    end;

    FLogger.Log(Format('Mise à jour de %s vers %s',
                      [CurrentVersion, UpdateVersion]));

    // 2. Arrêter le service
    if not StopService then
    begin
      FLogger.LogError('Impossible d''arrêter le service');
      Exit;
    end;
    ...
    begin
      FLogger.LogError('Impossible de créer la sauvegarde');
      StartService; // Redémarrer le service
      Exit;
    end;
    ...
    begin
      FLogger.LogError('Impossible d''appliquer la mise à jour');
      RollbackUpdate;
      StartService;
      Exit;
    end;
    ...
    begin
      FLogger.LogError('Impossible de redémarrer le service');
    ...
    begin
      FLogger.LogError('Le service ne fonctionne pas après la mise à jour');
      RollbackUpdate;
      StartService;
      Exit;
    end;

    FLogger.Log('Mise à jour réussie!');
    Result := True;

    // 7. Nettoyer les anciennes sauvegardes (garder les 3 dernières)
    CleanOldBackups(3);

  except
    on E: Exception do
    begin
      FLogger.LogError('Erreur lors de la mise à jour : ' + E.Message);
      RollbackUpdate;
      StartService;
    end;
  end;
...
begin
  ...
  if Result then
    FLogger.Log('Sauvegarde créée : ' + BackupDir);
end;
...
begin
  ...
  begin
    ...
    if CopyDirectory(LastBackup, ExtractFilePath(ParamStr(0))) then
      FLogger.Log('Rollback réussi depuis : ' + LastBackup)
    else
      FLogger.LogError('Échec du rollback');
  end
  else
    FLogger.LogError('Aucune sauvegarde trouvée pour le rollback');
end;
```
...
recovery_procedures:
  ...
  corruption_service:
    steps:
      1: Arrêter le service immédiatement
...
2. **Tu géreras les erreurs** : Toute exception doit être capturée et traitée
3. **Tu ne bloqueras point** : Utilise des timeouts et du code asynchrone
4. **Tu surveilleras la mémoire** : Les fuites mémoire tuent les services
...
8. **Tu sécuriseras ton service** : Principe du moindre privilège toujours
9. **Tu automatiseras le déploiement** : Les installations manuelles génèrent des erreurs
10. **Tu monitoreras activement** : Un service non surveillé tombera au pire moment
...
- **Outils de monitoring** :
  ...
  - Datadog (payant, très complet)
  - New Relic (payant, excellent pour les performances)

### Message final

Créer un service Windows robuste avec FreePascal/Lazarus demande de la rigueur et de la patience, mais le résultat en vaut la peine. Un service bien conçu peut tourner des années sans intervention, traitant silencieusement des millions d'opérations.

N'oubliez jamais : un service est comme un moteur de voiture - il doit être fiable, bien entretenu, et surveillé régulièrement. Avec les bonnes pratiques et les outils présentés dans ce tutoriel, vous avez tout ce qu'il faut pour créer des services Windows professionnels et robustes.

Bon développement !

⏭️ [Interaction avec le Service Control Manager](/06-specificites-windows/02.2-interaction-service-control-manager.md)
