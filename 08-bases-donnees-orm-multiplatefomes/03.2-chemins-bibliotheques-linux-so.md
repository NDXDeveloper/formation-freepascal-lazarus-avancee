🔝 Retour au [Sommaire](/SOMMAIRE.md)

# 8.3.2 Chemins de bibliothèques Linux (.so)

## Introduction aux bibliothèques partagées Linux

Sur Linux, les bibliothèques dynamiques sont appelées **bibliothèques partagées** (shared libraries) et utilisent l'extension `.so` (Shared Object). Ces bibliothèques sont l'équivalent des fichiers `.dll` de Windows, mais avec des conventions et une gestion différentes.

### Qu'est-ce qu'une bibliothèque partagée ?

Une bibliothèque partagée contient du code compilé qui peut être utilisé par plusieurs programmes simultanément. Au lieu d'incorporer ce code dans chaque programme, Linux le charge dynamiquement en mémoire, ce qui permet :

- **Économie de mémoire** : Une seule copie en mémoire pour tous les programmes
- **Mises à jour facilitées** : Corriger un bug dans la bibliothèque corrige tous les programmes
- **Taille réduite des exécutables** : Le code de la bibliothèque n'est pas dupliqué

### Convention de nommage

Les bibliothèques Linux suivent une convention de nommage stricte :

```
lib<nom>.so[.version][.révision][.age]
```

**Exemples :**
- `libfbclient.so` → Lien symbolique vers la version actuelle
- `libfbclient.so.3` → Version majeure 3
- `libfbclient.so.3.0.7` → Version complète 3.0.7
- `libmysqlclient.so.21` → Version majeure 21 de MySQL
- `libpq.so.5` → Version majeure 5 de PostgreSQL

**Pourquoi ce système de versionnement ?**

Le versionnement permet à plusieurs versions d'une même bibliothèque de coexister sur le système, garantissant la compatibilité des anciennes applications tout en permettant l'utilisation des nouvelles versions.

## Emplacements standards des bibliothèques

### Hiérarchie des répertoires système

Linux organise les bibliothèques selon une hiérarchie standard définie par le **FHS** (Filesystem Hierarchy Standard) :

#### 1. `/lib/` et `/lib64/`

**Utilisation :** Bibliothèques essentielles au démarrage du système

```bash
/lib/
├── x86_64-linux-gnu/     # Architecture 64 bits (Debian/Ubuntu)
├── i386-linux-gnu/       # Architecture 32 bits (si installé)
└── modules/              # Modules du noyau
```

**Exemples de bibliothèques :**
```bash
/lib/x86_64-linux-gnu/libc.so.6         # Bibliothèque C standard
/lib/x86_64-linux-gnu/libpthread.so.0   # Threads POSIX
```

**Note :** Sur les systèmes 64 bits modernes, `/lib64/` peut être un lien symbolique vers `/lib/`.

#### 2. `/usr/lib/` et `/usr/lib64/`

**Utilisation :** Bibliothèques des applications utilisateur (le plus courant)

```bash
/usr/lib/
├── x86_64-linux-gnu/     # Bibliothèques 64 bits
│   ├── libfbclient.so.3
│   ├── libmysqlclient.so.21
│   ├── libpq.so.5
│   └── libsqlite3.so.0
└── i386-linux-gnu/       # Bibliothèques 32 bits (si multilib)
```

**C'est ici que se trouvent la plupart des bibliothèques de bases de données !**

#### 3. `/usr/local/lib/`

**Utilisation :** Bibliothèques installées manuellement par l'administrateur

```bash
/usr/local/lib/
├── libcustomdb.so        # Bibliothèque compilée localement
└── libfirebird.so        # Version personnalisée de Firebird
```

**Quand l'utiliser ?**
- Compilation depuis les sources
- Versions personnalisées
- Bibliothèques non disponibles dans les dépôts

#### 4. `/opt/`

**Utilisation :** Applications tierces complètes avec leurs bibliothèques

```bash
/opt/
├── firebird/
│   └── lib/
│       └── libfbclient.so
└── oracle/
    └── lib/
        └── libclntsh.so
```

#### 5. Répertoire de l'application

Certaines applications embarquent leurs propres bibliothèques :

```bash
/home/utilisateur/MonApp/
├── MonApp                # Exécutable
├── libfbclient.so       # Bibliothèque embarquée
└── data/
```

### Architecture et multi-architecture

#### Debian/Ubuntu : système multi-arch

Debian et Ubuntu utilisent un système permettant d'installer plusieurs architectures :

```bash
# Architecture principale (64 bits)
/usr/lib/x86_64-linux-gnu/

# Architecture secondaire (32 bits, si installé)
/usr/lib/i386-linux-gnu/

# ARM 64 bits
/usr/lib/aarch64-linux-gnu/

# ARM 32 bits
/usr/lib/arm-linux-gnueabihf/
```

**Vérifier votre architecture :**
```bash
# Afficher l'architecture
dpkg --print-architecture
# Sortie typique : amd64

# Lister toutes les architectures installées
dpkg --print-foreign-architectures
# Sortie typique : i386 (si support 32 bits activé)

# Méthode alternative
uname -m
# Sortie : x86_64 (64 bits) ou i686 (32 bits)
```

#### Red Hat/Fedora : système lib64

Sur les distributions Red Hat, Fedora, CentOS :

```bash
/usr/lib/       # Bibliothèques 32 bits (historique)
/usr/lib64/     # Bibliothèques 64 bits
```

## Chemins spécifiques par base de données

### Firebird

**Paquets Ubuntu/Debian :**
```bash
sudo apt install firebird3.0-server        # Serveur
sudo apt install firebird3.0-client        # Client uniquement
sudo apt install firebird3.0-utils         # Outils
```

**Emplacements des bibliothèques :**

```bash
# Bibliothèque cliente principale
/usr/lib/x86_64-linux-gnu/libfbclient.so.2

# Lien symbolique vers la version actuelle
/usr/lib/x86_64-linux-gnu/libfbclient.so -> libfbclient.so.2

# Bibliothèque embedded (tout-en-un)
/usr/lib/x86_64-linux-gnu/libfbembed.so.2.5

# Plugin moderne (Firebird 3+)
/usr/lib/x86_64-linux-gnu/firebird/3.0/plugins/
```

**Vérification de l'installation :**
```bash
# Trouver les fichiers Firebird
dpkg -L firebird3.0-client | grep libfbclient

# Vérifier les liens symboliques
ls -l /usr/lib/x86_64-linux-gnu/libfbclient*
```

**Configuration dans FreePascal :**
```pascal
{$IFDEF LINUX}
IBConnection1.Params.Add('ClientLibrary=/usr/lib/x86_64-linux-gnu/libfbclient.so');
// ou simplement
IBConnection1.Params.Add('ClientLibrary=libfbclient.so');  // Si dans le path
{$ENDIF}
```

### MySQL/MariaDB

**Paquets Ubuntu/Debian :**
```bash
# MySQL
sudo apt install libmysqlclient-dev        # Bibliothèque de développement
sudo apt install libmysqlclient21          # Bibliothèque runtime

# MariaDB (alternative compatible)
sudo apt install libmariadb-dev
sudo apt install libmariadb3
```

**Emplacements des bibliothèques :**

```bash
# MySQL
/usr/lib/x86_64-linux-gnu/libmysqlclient.so.21
/usr/lib/x86_64-linux-gnu/libmysqlclient.so -> libmysqlclient.so.21

# MariaDB
/usr/lib/x86_64-linux-gnu/libmariadb.so.3
/usr/lib/x86_64-linux-gnu/libmariadb.so -> libmariadb.so.3
```

**Vérification :**
```bash
# MySQL
dpkg -L libmysqlclient21 | grep "\.so"

# Vérifier la version
ls -l /usr/lib/x86_64-linux-gnu/libmysqlclient*
```

**Configuration dans FreePascal :**
```pascal
{$IFDEF LINUX}
MySQLConnection1.Params.Add('ClientLibrary=/usr/lib/x86_64-linux-gnu/libmysqlclient.so');
// ou pour MariaDB
MySQLConnection1.Params.Add('ClientLibrary=/usr/lib/x86_64-linux-gnu/libmariadb.so');
{$ENDIF}
```

### PostgreSQL

**Paquets Ubuntu/Debian :**
```bash
sudo apt install libpq-dev                 # Bibliothèque de développement
sudo apt install libpq5                    # Bibliothèque runtime (version 5)
```

**Emplacements des bibliothèques :**

```bash
# Bibliothèque principale
/usr/lib/x86_64-linux-gnu/libpq.so.5
/usr/lib/x86_64-linux-gnu/libpq.so -> libpq.so.5

# Versions spécifiques PostgreSQL
/usr/lib/postgresql/14/lib/
/usr/lib/postgresql/15/lib/
```

**Vérification :**
```bash
dpkg -L libpq5 | grep "\.so"

# Vérifier toutes les versions PostgreSQL installées
ls -l /usr/lib/postgresql/*/lib/
```

**Configuration dans FreePascal :**
```pascal
{$IFDEF LINUX}
PQConnection1.Params.Add('ClientLibrary=/usr/lib/x86_64-linux-gnu/libpq.so');
{$ENDIF}
```

### SQLite

**Paquets Ubuntu/Debian :**
```bash
sudo apt install libsqlite3-0              # Bibliothèque runtime
sudo apt install libsqlite3-dev            # Fichiers de développement
```

**Emplacements des bibliothèques :**

```bash
# Bibliothèque principale
/usr/lib/x86_64-linux-gnu/libsqlite3.so.0
/usr/lib/x86_64-linux-gnu/libsqlite3.so -> libsqlite3.so.0
```

**Vérification :**
```bash
dpkg -L libsqlite3-0 | grep "\.so"
```

**Configuration dans FreePascal :**
```pascal
{$IFDEF LINUX}
SQLite3Connection1.Params.Add('ClientLibrary=/usr/lib/x86_64-linux-gnu/libsqlite3.so');
{$ENDIF}
```

## Mécanisme de recherche des bibliothèques

### Le linker dynamique (ld.so)

Linux utilise le **linker dynamique** (`ld.so` ou `ld-linux.so`) pour charger les bibliothèques au lancement d'un programme.

**Ordre de recherche par défaut :**

1. **Répertoires dans `RPATH`** (chemin compilé dans l'exécutable)
2. **Variable d'environnement `LD_LIBRARY_PATH`**
3. **Répertoires dans `RUNPATH`** (chemin compilé, priorité basse)
4. **Cache du linker** (`/etc/ld.so.cache`)
5. **Répertoires par défaut** (`/lib`, `/usr/lib`)

### Variable d'environnement LD_LIBRARY_PATH

`LD_LIBRARY_PATH` permet de spécifier des répertoires supplémentaires pour la recherche :

**Utilisation temporaire (session actuelle) :**
```bash
# Ajouter un répertoire
export LD_LIBRARY_PATH=/opt/firebird/lib:$LD_LIBRARY_PATH

# Vérifier
echo $LD_LIBRARY_PATH

# Lancer l'application
./MonApplication
```

**Utilisation permanente (utilisateur) :**
```bash
# Éditer ~/.bashrc
nano ~/.bashrc

# Ajouter à la fin
export LD_LIBRARY_PATH=/opt/firebird/lib:$LD_LIBRARY_PATH

# Recharger la configuration
source ~/.bashrc
```

**Utilisation permanente (système) :**
```bash
# Créer un fichier de configuration
sudo nano /etc/ld.so.conf.d/firebird.conf

# Ajouter le chemin
/opt/firebird/lib

# Mettre à jour le cache
sudo ldconfig
```

**⚠️ Attention :** L'utilisation excessive de `LD_LIBRARY_PATH` peut causer des conflits. Préférez la configuration système via `ldconfig`.

### Configuration avec ldconfig

`ldconfig` est l'outil qui gère le cache des bibliothèques partagées.

**Ajouter un répertoire de bibliothèques :**

```bash
# Méthode 1 : Créer un fichier de configuration
sudo nano /etc/ld.so.conf.d/custom.conf

# Ajouter le chemin
/usr/local/lib/firebird

# Mettre à jour le cache
sudo ldconfig
```

**Vérifier qu'une bibliothèque est dans le cache :**
```bash
# Rechercher Firebird
ldconfig -p | grep firebird

# Sortie exemple :
# libfbclient.so.2 (libc6,x86-64) => /usr/lib/x86_64-linux-gnu/libfbclient.so.2
```

**Lister toutes les bibliothèques :**
```bash
ldconfig -p
```

**Afficher les bibliothèques utilisées par un programme :**
```bash
# Voir les dépendances
ldd /usr/bin/isql-fb

# Sortie exemple :
# libfbclient.so.2 => /usr/lib/x86_64-linux-gnu/libfbclient.so.2
# libc.so.6 => /lib/x86_64-linux-gnu/libc.so.6
```

### Liens symboliques

Linux utilise massivement les liens symboliques pour gérer les versions :

**Exemple avec Firebird :**
```bash
ls -l /usr/lib/x86_64-linux-gnu/libfbclient*

# Sortie :
# lrwxrwxrwx libfbclient.so -> libfbclient.so.2
# lrwxrwxrwx libfbclient.so.2 -> libfbclient.so.2.5.9
# -rw-r--r-- libfbclient.so.2.5.9
```

**Hiérarchie des liens :**
1. `libfbclient.so` → Lien pour le développement
2. `libfbclient.so.2` → Lien pour la version majeure
3. `libfbclient.so.2.5.9` → Fichier réel

**Créer un lien symbolique manuellement :**
```bash
# Créer un lien
sudo ln -s /usr/lib/x86_64-linux-gnu/libfbclient.so.2 /usr/local/lib/libfbclient.so

# Vérifier
ls -l /usr/local/lib/libfbclient.so

# Mettre à jour le cache
sudo ldconfig
```

## Détection automatique dans FreePascal

### Fonction de recherche multi-emplacements

```pascal
function FindLinuxLibrary(const LibName: string): string;
var
  SearchPaths: array of string;
  Path: string;
  I: Integer;
begin
  Result := '';

  {$IFDEF LINUX}
  // Construire la liste des chemins à chercher
  SetLength(SearchPaths, 6);

  // Répertoire de l'application
  SearchPaths[0] := ExtractFilePath(ParamStr(0)) + LibName;

  // Chemins système standard pour architecture 64 bits
  SearchPaths[1] := '/usr/lib/x86_64-linux-gnu/' + LibName;
  SearchPaths[2] := '/usr/lib/' + LibName;
  SearchPaths[3] := '/usr/local/lib/' + LibName;

  // Chemin optionnel
  SearchPaths[4] := '/opt/firebird/lib/' + LibName;

  // Laisser le système chercher (nom seul)
  SearchPaths[5] := LibName;

  // Chercher dans tous les emplacements
  for I := 0 to High(SearchPaths) do
  begin
    Path := SearchPaths[I];

    // Pour le dernier cas (nom seul), on retourne directement
    if I = High(SearchPaths) then
    begin
      Result := Path;
      Exit;
    end;

    // Vérifier si le fichier existe
    if FileExists(Path) then
    begin
      Result := Path;
      Exit;
    end;
  end;
  {$ENDIF}
end;
```

**Utilisation :**
```pascal
var
  FirebirdLib: string;
begin
  FirebirdLib := FindLinuxLibrary('libfbclient.so.2');

  if FirebirdLib <> '' then
  begin
    IBConnection1.Params.Add('ClientLibrary=' + FirebirdLib);
    WriteLn('Bibliothèque trouvée : ', FirebirdLib);
  end
  else
    raise Exception.Create('Bibliothèque Firebird introuvable');
end;
```

### Détection de l'architecture

```pascal
function GetLinuxArchitecture: string;
var
  Process: TProcess;
  OutputList: TStringList;
begin
  Result := 'x86_64-linux-gnu';  // Valeur par défaut

  {$IFDEF LINUX}
  Process := TProcess.Create(nil);
  OutputList := TStringList.Create;
  try
    Process.Executable := 'dpkg';
    Process.Parameters.Add('--print-architecture');
    Process.Options := [poWaitOnExit, poUsePipes];
    Process.Execute;

    OutputList.LoadFromStream(Process.Output);

    if OutputList.Count > 0 then
    begin
      // Convertir l'architecture Debian en chemin de bibliothèque
      case Trim(OutputList[0]) of
        'amd64': Result := 'x86_64-linux-gnu';
        'i386': Result := 'i386-linux-gnu';
        'arm64': Result := 'aarch64-linux-gnu';
        'armhf': Result := 'arm-linux-gnueabihf';
      end;
    end;
  finally
    OutputList.Free;
    Process.Free;
  end;
  {$ENDIF}
end;
```

**Utilisation :**
```pascal
var
  Arch, LibPath: string;
begin
  Arch := GetLinuxArchitecture;
  LibPath := '/usr/lib/' + Arch + '/libfbclient.so';

  WriteLn('Architecture : ', Arch);
  WriteLn('Chemin bibliothèque : ', LibPath);
end;
```

### Vérification des dépendances

```pascal
function CheckLibraryDependencies(const LibPath: string): TStringList;
var
  Process: TProcess;
begin
  Result := TStringList.Create;

  {$IFDEF LINUX}
  Process := TProcess.Create(nil);
  try
    Process.Executable := 'ldd';
    Process.Parameters.Add(LibPath);
    Process.Options := [poWaitOnExit, poUsePipes];
    Process.Execute;

    Result.LoadFromStream(Process.Output);
  finally
    Process.Free;
  end;
  {$ENDIF}
end;
```

**Utilisation :**
```pascal
var
  Dependencies: TStringList;
begin
  Dependencies := CheckLibraryDependencies('/usr/lib/x86_64-linux-gnu/libfbclient.so');
  try
    WriteLn('Dépendances de libfbclient.so :');
    WriteLn(Dependencies.Text);
  finally
    Dependencies.Free;
  end;
end;
```

## Gestion des permissions

### Permissions des bibliothèques

Les bibliothèques Linux ont des permissions spécifiques :

```bash
# Afficher les permissions
ls -l /usr/lib/x86_64-linux-gnu/libfbclient.so.2

# Sortie typique :
# -rw-r--r-- 1 root root 584616 Jan 15 10:30 libfbclient.so.2
```

**Explication des permissions :**
- `rw-` : Le propriétaire (root) peut lire et écrire
- `r--` : Le groupe peut lire
- `r--` : Les autres peuvent lire

**Modifier les permissions (si nécessaire) :**
```bash
# Rendre exécutable (rarement nécessaire pour .so)
sudo chmod 755 /usr/local/lib/custom.so

# Permissions standards pour une bibliothèque
sudo chmod 644 /usr/local/lib/custom.so
```

### Propriétaire et groupe

```bash
# Vérifier le propriétaire
ls -l /usr/lib/x86_64-linux-gnu/libfbclient.so.2

# Changer le propriétaire (si nécessaire)
sudo chown root:root /usr/local/lib/custom.so
```

## Débogage des problèmes de bibliothèques

### Problème : "Cannot load library"

**Erreur typique :**
```
Error: Cannot load library "libfbclient.so": libfbclient.so:
cannot open shared object file: No such file or directory
```

**Solutions :**

1. **Vérifier que la bibliothèque est installée :**
```bash
dpkg -l | grep firebird
sudo apt install firebird3.0-client
```

2. **Vérifier le chemin :**
```bash
find /usr -name "libfbclient.so*" 2>/dev/null
```

3. **Vérifier le cache ldconfig :**
```bash
ldconfig -p | grep fbclient
```

4. **Ajouter au cache si nécessaire :**
```bash
sudo nano /etc/ld.so.conf.d/firebird.conf
# Ajouter : /usr/lib/x86_64-linux-gnu
sudo ldconfig
```

### Problème : Version incompatible

**Erreur typique :**
```
Error: /usr/lib/libfbclient.so.2: version 'FBCLIENT_2.5' not found
```

**Solutions :**

1. **Vérifier les versions disponibles :**
```bash
ls -l /usr/lib/x86_64-linux-gnu/libfbclient*
```

2. **Installer la bonne version :**
```bash
sudo apt install firebird3.0-client
```

3. **Créer un lien symbolique (temporaire) :**
```bash
sudo ln -s /usr/lib/x86_64-linux-gnu/libfbclient.so.3 /usr/lib/libfbclient.so.2
```

### Problème : Architecture incorrecte

**Erreur typique :**
```
Error: wrong ELF class: ELFCLASS32
```

**Cause :** Tentative d'utiliser une bibliothèque 32 bits avec un programme 64 bits (ou inverse).

**Solutions :**

1. **Vérifier l'architecture de votre programme :**
```bash
file MonApplication
# Sortie : ELF 64-bit LSB executable
```

2. **Vérifier l'architecture de la bibliothèque :**
```bash
file /usr/lib/x86_64-linux-gnu/libfbclient.so.2
# Sortie : ELF 64-bit LSB shared object
```

3. **Installer la bonne architecture :**
```bash
# Pour 64 bits
sudo apt install firebird3.0-client:amd64

# Pour 32 bits
sudo dpkg --add-architecture i386
sudo apt update
sudo apt install firebird3.0-client:i386
```

### Outil de débogage : strace

`strace` permet de voir tous les appels système, y compris la recherche de bibliothèques :

```bash
# Tracer les appels à open() pour voir les tentatives de chargement
strace -e open,openat ./MonApplication 2>&1 | grep "\.so"

# Sortie exemple :
# openat(AT_FDCWD, "/usr/lib/x86_64-linux-gnu/libfbclient.so.2", O_RDONLY) = 3
```

## Configuration centralisée pour application multi-base

### Module de configuration réutilisable

```pascal
unit LinuxDatabaseLibs;

{$mode objfpc}{$H+}

interface

uses
  Classes, SysUtils;

type
  TDatabaseType = (dtFirebird, dtMySQL, dtPostgreSQL, dtSQLite);

  TLinuxLibConfig = record
    LibraryName: string;
    SearchPaths: array of string;
    PackageName: string;
  end;

function GetLinuxLibraryPath(DBType: TDatabaseType): string;
function GetInstallCommand(DBType: TDatabaseType): string;

implementation

function GetLibConfig(DBType: TDatabaseType): TLinuxLibConfig;
var
  Arch: string;
begin
  Arch := 'x86_64-linux-gnu';  // Simplification, à adapter

  case DBType of
    dtFirebird:
    begin
      Result.LibraryName := 'libfbclient.so.2';
      SetLength(Result.SearchPaths, 3);
      Result.SearchPaths[0] := '/usr/lib/' + Arch + '/';
      Result.SearchPaths[1] := '/usr/local/lib/';
      Result.SearchPaths[2] := '/opt/firebird/lib/';
      Result.PackageName := 'firebird3.0-client';
    end;

    dtMySQL:
    begin
      Result.LibraryName := 'libmysqlclient.so.21';
      SetLength(Result.SearchPaths, 2);
      Result.SearchPaths[0] := '/usr/lib/' + Arch + '/';
      Result.SearchPaths[1] := '/usr/local/lib/';
      Result.PackageName := 'libmysqlclient21';
    end;

    dtPostgreSQL:
    begin
      Result.LibraryName := 'libpq.so.5';
      SetLength(Result.SearchPaths, 2);
      Result.SearchPaths[0] := '/usr/lib/' + Arch + '/';
      Result.SearchPaths[1] := '/usr/local/lib/';
      Result.PackageName := 'libpq5';
    end;

    dtSQLite:
    begin
      Result.LibraryName := 'libsqlite3.so.0';
      SetLength(Result.SearchPaths, 2);
      Result.SearchPaths[0] := '/usr/lib/' + Arch + '/';
      Result.SearchPaths[1] := '/usr/local/lib/';
      Result.PackageName := 'libsqlite3-0';
    end;
  end;
end;

function GetLinuxLibraryPath(DBType: TDatabaseType): string;
var
  Config: TLinuxLibConfig;
  Path: string;
  FullPath: string;
begin
  Result := '';
  Config := GetLibConfig(DBType);

  // Chercher dans tous les emplacements
  for Path in Config.SearchPaths do
  begin
    FullPath := Path + Config.LibraryName;
    if FileExists(FullPath) then
    begin
      Result := FullPath;
      Exit;
    end;
  end;

  // Si non trouvé, retourner juste le nom (recherche système)
  Result := Config.LibraryName;
end;

function GetInstallCommand(DBType: TDatabaseType): string;
var
  Config: TLinuxLibConfig;
begin
  Config := GetLibConfig(DBType);
  Result := 'sudo apt install ' + Config.PackageName;
end;

end.
```

**Utilisation dans votre application :**

```pascal
uses
  LinuxDatabaseLibs;

procedure ConfigurerFirebird;
var
  LibPath: string;
begin
  {$IFDEF LINUX}
  LibPath := GetLinuxLibraryPath(dtFirebird);

  if not FileExists(LibPath) then
  begin
    ShowMessage('Bibliothèque Firebird non trouvée. Installez avec : ' +
                GetInstallCommand(dtFirebird));
    Exit;
  end;

  IBConnection1.Params.Add('ClientLibrary=' + LibPath);
  WriteLn('Firebird configuré : ', LibPath);
  {$ENDIF}
end;
```

## Bonnes pratiques Linux

### 1. Utiliser les paquets officiels

✅ **RECOMMANDÉ :**
```bash
sudo apt install firebird3.0-client
```

❌ **À ÉVITER (sauf nécessité) :**
```bash
# Télécharger et compiler depuis les sources
```

**Pourquoi ?** Les paquets officiels :
- Sont testés et stables
- Se mettent à jour automatiquement
- Respectent les conventions système
- Gèrent les dépendances

### 2. Ne pas utiliser LD_LIBRARY_PATH en production

✅ **RECOMMANDÉ :**
```bash
sudo nano /etc/ld.so.conf.d/custom.conf
sudo ldconfig
```

❌ **À ÉVITER en production :**
```bash
export LD_LIBRARY_PATH=/custom/path:$LD_LIBRARY_PATH
```

**Pourquoi ?** `LD_LIBRARY_PATH` peut :
- Causer des conflits de versions
- Être oublié lors du déploiement
- Affecter tous les programmes

### 3. Vérifier les dépendances avant distribution

```bash
# Liste complète des bibliothèques nécessaires
ldd ./MonApplication

# Vérifier qu'elles sont toutes disponibles
ldd ./MonApplication | grep "not found"
```

Si cette commande affiche des résultats, des bibliothèques manquent.

### 4. Documenter les dépendances

Créez un fichier `INSTALL.md` ou `README.md` :

````markdown
## Installation des dépendances (Ubuntu/Debian)

```bash
# Firebird
sudo apt install firebird3.0-client

# MySQL
sudo apt install libmysqlclient21

# PostgreSQL
sudo apt install libpq5

# SQLite
sudo apt install libsqlite3-0
```

## Installation des dépendances (Red Hat/Fedora)

```bash
sudo dnf install firebird-libfbclient
sudo dnf install mysql-libs
sudo dnf install postgresql-libs
sudo dnf install sqlite-libs
```
````

### 5. Tester sur différentes distributions

Les chemins peuvent varier selon la distribution :

| Distribution | Firebird |
|-------------|----------|
| Ubuntu/Debian | `/usr/lib/x86_64-linux-gnu/libfbclient.so.2` |
| Fedora/RHEL | `/usr/lib64/libfbclient.so.2` |
| Arch Linux | `/usr/lib/libfbclient.so` |
| openSUSE | `/usr/lib64/libfbclient.so.2` |

**Solution :** Utiliser la recherche automatique plutôt que des chemins codés en dur.

### 6. Gérer les versions multiples

Si plusieurs versions doivent coexister :

```bash
# Installer une version spécifique
sudo apt install firebird3.0-client=3.0.7.33374-0ubuntu0.20.04.1

# Empêcher les mises à jour automatiques
sudo apt-mark hold firebird3.0-client

# Permettre à nouveau les mises à jour
sudo apt-mark unhold firebird3.0-client
```

### 7. Utiliser alternatives pour gérer les versions

```bash
# Créer des alternatives pour plusieurs versions
sudo update-alternatives --install /usr/lib/libfbclient.so libfbclient \
    /usr/lib/x86_64-linux-gnu/libfbclient.so.2 20

sudo update-alternatives --install /usr/lib/libfbclient.so libfbclient \
    /usr/lib/x86_64-linux-gnu/libfbclient.so.3 30

# Choisir la version à utiliser
sudo update-alternatives --config libfbclient

# Afficher la version active
update-alternatives --display libfbclient
```

## Déploiement d'applications

### Méthode 1 : Installation des paquets (recommandé)

**Script d'installation :**

```bash
#!/bin/bash
# install.sh

echo "Installation des dépendances..."

# Détecter la distribution
if [ -f /etc/debian_version ]; then
    # Debian/Ubuntu
    sudo apt update
    sudo apt install -y firebird3.0-client libmysqlclient21 libpq5
elif [ -f /etc/redhat-release ]; then
    # Red Hat/Fedora/CentOS
    sudo dnf install -y firebird-libfbclient mysql-libs postgresql-libs
elif [ -f /etc/arch-release ]; then
    # Arch Linux
    sudo pacman -S --noconfirm libfbclient libmysqlclient postgresql-libs
else
    echo "Distribution non supportée"
    exit 1
fi

echo "Installation terminée !"
```

**Rendre le script exécutable :**
```bash
chmod +x install.sh
```

### Méthode 2 : Bibliothèques embarquées

Pour une application autonome, embarquez les bibliothèques :

**Structure du déploiement :**
```
MonApplication/
├── MonApplication                    # Exécutable
├── lib/                             # Bibliothèques embarquées
│   ├── libfbclient.so.2
│   ├── libmysqlclient.so.21
│   └── libpq.so.5
├── data/                            # Données
└── run.sh                           # Script de lancement
```

**Script de lancement (run.sh) :**
```bash
#!/bin/bash

# Obtenir le répertoire du script
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Ajouter le répertoire lib au chemin de recherche
export LD_LIBRARY_PATH="$DIR/lib:$LD_LIBRARY_PATH"

# Lancer l'application
"$DIR/MonApplication" "$@"
```

**Configuration dans FreePascal pour utiliser lib/ :**
```pascal
function GetEmbeddedLibPath(const LibName: string): string;
var
  AppDir: string;
begin
  AppDir := ExtractFilePath(ParamStr(0));
  Result := AppDir + 'lib/' + LibName;

  if not FileExists(Result) then
    Result := LibName;  // Fallback sur recherche système
end;

// Utilisation
{$IFDEF LINUX}
IBConnection1.Params.Add('ClientLibrary=' + GetEmbeddedLibPath('libfbclient.so.2'));
{$ENDIF}
```

### Méthode 3 : AppImage

AppImage permet de distribuer une application Linux complète en un seul fichier :

**Structure AppImage :**
```
AppDir/
├── AppRun                           # Script de lancement
├── MonApplication.desktop           # Fichier desktop
├── MonApplication.png               # Icône
├── usr/
│   ├── bin/
│   │   └── MonApplication          # Votre exécutable
│   └── lib/
│       ├── libfbclient.so.2
│       └── ...
```

**Script AppRun :**
```bash
#!/bin/bash

SELF=$(readlink -f "$0")
HERE=${SELF%/*}

export LD_LIBRARY_PATH="${HERE}/usr/lib:${LD_LIBRARY_PATH}"
export PATH="${HERE}/usr/bin:${PATH}"

exec "${HERE}/usr/bin/MonApplication" "$@"
```

**Créer l'AppImage :**
```bash
# Télécharger l'outil
wget https://github.com/AppImage/AppImageKit/releases/download/continuous/appimagetool-x86_64.AppImage
chmod +x appimagetool-x86_64.AppImage

# Créer l'AppImage
./appimagetool-x86_64.AppImage AppDir MonApplication.AppImage
```

### Méthode 4 : Paquet DEB

Créer un paquet Debian pour faciliter l'installation :

**Structure du projet :**
```
monapp_1.0-1/
├── DEBIAN/
│   ├── control              # Métadonnées
│   ├── postinst            # Script post-installation
│   └── prerm               # Script pré-désinstallation
└── usr/
    ├── bin/
    │   └── monapp
    └── share/
        └── monapp/
            └── data/
```

**Fichier control :**
```
Package: monapp
Version: 1.0-1
Section: database
Priority: optional
Architecture: amd64
Depends: firebird3.0-client, libmysqlclient21, libpq5
Maintainer: Votre Nom <email@example.com>
Description: Mon Application de Base de Données
 Description longue de l'application
```

**Créer le paquet :**
```bash
dpkg-deb --build monapp_1.0-1
```

**Installer :**
```bash
sudo dpkg -i monapp_1.0-1.deb
sudo apt-get install -f  # Résoudre les dépendances
```

## Tests et validation

### Script de test des bibliothèques

```bash
#!/bin/bash
# test_libs.sh

echo "Test des bibliothèques de bases de données..."
echo "=============================================="

# Firebird
echo -n "Firebird: "
if ldconfig -p | grep -q "libfbclient.so"; then
    LIB=$(ldconfig -p | grep "libfbclient.so" | awk '{print $NF}' | head -1)
    echo "✓ Trouvé - $LIB"
else
    echo "✗ Non trouvé"
fi

# MySQL
echo -n "MySQL: "
if ldconfig -p | grep -q "libmysqlclient.so"; then
    LIB=$(ldconfig -p | grep "libmysqlclient.so" | awk '{print $NF}' | head -1)
    echo "✓ Trouvé - $LIB"
else
    echo "✗ Non trouvé"
fi

# PostgreSQL
echo -n "PostgreSQL: "
if ldconfig -p | grep -q "libpq.so"; then
    LIB=$(ldconfig -p | grep "libpq.so" | awk '{print $NF}' | head -1)
    echo "✓ Trouvé - $LIB"
else
    echo "✗ Non trouvé"
fi

# SQLite
echo -n "SQLite: "
if ldconfig -p | grep -q "libsqlite3.so"; then
    LIB=$(ldconfig -p | grep "libsqlite3.so" | awk '{print $NF}' | head -1)
    echo "✓ Trouvé - $LIB"
else
    echo "✗ Non trouvé"
fi

echo "=============================================="
```

### Programme FreePascal de test

```pascal
program TestLibs;

{$mode objfpc}{$H+}

uses
  SysUtils;

function TestLibrary(const LibName, DisplayName: string): Boolean;
var
  Handle: TLibHandle;
begin
  Result := False;
  Write(DisplayName, ': ');

  Handle := LoadLibrary(LibName);
  if Handle <> 0 then
  begin
    WriteLn('✓ OK');
    FreeLibrary(Handle);
    Result := True;
  end
  else
    WriteLn('✗ ERREUR - ', GetLoadErrorStr);
end;

var
  AllOK: Boolean;
begin
  WriteLn('Test de chargement des bibliothèques');
  WriteLn('====================================');

  AllOK := True;

  {$IFDEF LINUX}
  AllOK := TestLibrary('libfbclient.so.2', 'Firebird') and AllOK;
  AllOK := TestLibrary('libmysqlclient.so.21', 'MySQL') and AllOK;
  AllOK := TestLibrary('libpq.so.5', 'PostgreSQL') and AllOK;
  AllOK := TestLibrary('libsqlite3.so.0', 'SQLite') and AllOK;
  {$ENDIF}

  WriteLn('====================================');

  if AllOK then
  begin
    WriteLn('Toutes les bibliothèques sont disponibles !');
    Halt(0);
  end
  else
  begin
    WriteLn('Certaines bibliothèques sont manquantes.');
    Halt(1);
  end;
end.
```

**Compilation et exécution :**
```bash
fpc TestLibs.pas
./TestLibs
```

## Cas particuliers et situations avancées

### Bibliothèques dans des emplacements non standard

Si une bibliothèque est dans un emplacement inhabituel :

**Option 1 : RPATH (compilation) :**
```bash
# Compiler avec RPATH
fpc -k-rpath -k/opt/custom/lib MonApplication.pas
```

**Option 2 : Wrapper dynamique :**
```pascal
function LoadDynamicLibrary(const LibName: string): TLibHandle;
var
  CustomPaths: array[0..2] of string;
  I: Integer;
  FullPath: string;
begin
  Result := 0;

  CustomPaths[0] := '/opt/custom/lib/';
  CustomPaths[1] := '/usr/local/custom/';
  CustomPaths[2] := ExtractFilePath(ParamStr(0)) + 'lib/';

  for I := Low(CustomPaths) to High(CustomPaths) do
  begin
    FullPath := CustomPaths[I] + LibName;
    if FileExists(FullPath) then
    begin
      Result := LoadLibrary(FullPath);
      if Result <> 0 then Exit;
    end;
  end;

  // Tentative avec le nom seul (recherche système)
  Result := LoadLibrary(LibName);
end;
```

### Utilisation de versions multiples simultanément

Parfois, différentes parties d'une application doivent utiliser différentes versions :

```pascal
type
  TFirebirdVersion = (fbV2, fbV3, fbV4);

function GetFirebirdLibrary(Version: TFirebirdVersion): string;
begin
  case Version of
    fbV2: Result := '/usr/lib/x86_64-linux-gnu/libfbclient.so.2';
    fbV3: Result := '/usr/lib/x86_64-linux-gnu/libfbclient.so.3';
    fbV4: Result := '/usr/lib/x86_64-linux-gnu/libfbclient.so.4';
  end;
end;

// Utilisation avec des connexions différentes
procedure ConfigurerConnexions;
begin
  // Connexion vers base Firebird 2.5
  IBConnection1.Params.Add('ClientLibrary=' + GetFirebirdLibrary(fbV2));

  // Connexion vers base Firebird 3.0
  IBConnection2.Params.Add('ClientLibrary=' + GetFirebirdLibrary(fbV3));
end;
```

### Environnements conteneurisés (Docker)

Dans un conteneur Docker, simplifiez la gestion :

**Dockerfile :**
```dockerfile
FROM ubuntu:22.04

# Installation des dépendances
RUN apt-get update && apt-get install -y \
    firebird3.0-client \
    libmysqlclient21 \
    libpq5 \
    libsqlite3-0 \
    && rm -rf /var/lib/apt/lists/*

# Copier l'application
COPY MonApplication /app/
WORKDIR /app

# Lancer l'application
CMD ["./MonApplication"]
```

Dans ce cas, pas besoin de configuration spéciale, les bibliothèques sont au bon endroit.

## Outils de diagnostic

### Script complet de diagnostic

```bash
#!/bin/bash
# diagnose_libs.sh

echo "=== Diagnostic des bibliothèques de bases de données ==="
echo ""

# Fonction pour tester une bibliothèque
check_lib() {
    local lib_pattern=$1
    local lib_name=$2

    echo "--- $lib_name ---"

    # Recherche dans ldconfig
    echo "Cache ldconfig:"
    ldconfig -p | grep "$lib_pattern" || echo "  Non trouvé dans le cache"
    echo ""

    # Recherche dans le système de fichiers
    echo "Fichiers système:"
    find /usr/lib /usr/local/lib -name "*${lib_pattern}*" 2>/dev/null || echo "  Aucun fichier trouvé"
    echo ""

    # Paquet Debian associé
    echo "Paquet Debian/Ubuntu:"
    dpkg -l | grep -i "$lib_pattern" 2>/dev/null || echo "  Aucun paquet installé"
    echo ""
}

# Architecture système
echo "Architecture:"
uname -m
dpkg --print-architecture 2>/dev/null || echo "dpkg non disponible"
echo ""

# Tests par bibliothèque
check_lib "libfbclient" "Firebird"
check_lib "libmysqlclient\|libmariadb" "MySQL/MariaDB"
check_lib "libpq" "PostgreSQL"
check_lib "libsqlite3" "SQLite"

# Variables d'environnement
echo "--- Variables d'environnement ---"
echo "LD_LIBRARY_PATH: ${LD_LIBRARY_PATH:-<non défini>}"
echo ""

# Configuration ldconfig
echo "--- Configuration ldconfig ---"
echo "Fichiers de configuration:"
ls -l /etc/ld.so.conf.d/ 2>/dev/null || echo "Répertoire non accessible"
echo ""

echo "=== Fin du diagnostic ==="
```

**Utilisation :**
```bash
chmod +x diagnose_libs.sh
./diagnose_libs.sh > diagnostic.txt
```

## Tableau récapitulatif des chemins

| Base de données | Bibliothèque | Chemin typique Ubuntu/Debian | Paquet |
|----------------|--------------|----------------------------|---------|
| **Firebird 2.5** | `libfbclient.so.2` | `/usr/lib/x86_64-linux-gnu/` | `firebird2.5-client` |
| **Firebird 3.0** | `libfbclient.so.3` | `/usr/lib/x86_64-linux-gnu/` | `firebird3.0-client` |
| **MySQL 5.7** | `libmysqlclient.so.20` | `/usr/lib/x86_64-linux-gnu/` | `libmysqlclient20` |
| **MySQL 8.0** | `libmysqlclient.so.21` | `/usr/lib/x86_64-linux-gnu/` | `libmysqlclient21` |
| **MariaDB** | `libmariadb.so.3` | `/usr/lib/x86_64-linux-gnu/` | `libmariadb3` |
| **PostgreSQL 12** | `libpq.so.5` | `/usr/lib/x86_64-linux-gnu/` | `libpq5` |
| **PostgreSQL 14+** | `libpq.so.5` | `/usr/lib/x86_64-linux-gnu/` | `libpq5` |
| **SQLite 3** | `libsqlite3.so.0` | `/usr/lib/x86_64-linux-gnu/` | `libsqlite3-0` |

## Ressources supplémentaires

### Documentation Linux

- **Manual pages :**
  ```bash
  man ldconfig
  man ld.so
  man ldd
  man dlopen
  ```

- **FHS (Filesystem Hierarchy Standard) :**
  https://refspecs.linuxfoundation.org/FHS_3.0/fhs/index.html

### Outils utiles

```bash
# Installation des outils de développement
sudo apt install build-essential

# Installation d'outils de diagnostic
sudo apt install binutils ltrace strace

# Afficher les symboles d'une bibliothèque
nm -D /usr/lib/x86_64-linux-gnu/libfbclient.so.2

# Afficher les sections d'une bibliothèque
objdump -x /usr/lib/x86_64-linux-gnu/libfbclient.so.2

# Vérifier les dépendances
ldd /usr/lib/x86_64-linux-gnu/libfbclient.so.2
```

### Documentation des bases de données

- **Firebird :** https://firebirdsql.org/en/reference-manuals/
- **MySQL :** https://dev.mysql.com/doc/
- **PostgreSQL :** https://www.postgresql.org/docs/
- **SQLite :** https://www.sqlite.org/docs.html

## Conclusion

La gestion des bibliothèques partagées sous Linux est un aspect fondamental du développement multi-plateforme. Les points clés à retenir :

✅ **Utiliser les chemins standards** : `/usr/lib/x86_64-linux-gnu/` pour Debian/Ubuntu

✅ **Privilégier les paquets officiels** : Installation via `apt` ou équivalent

✅ **Configurer ldconfig** : Pour les bibliothèques dans des emplacements personnalisés

✅ **Éviter LD_LIBRARY_PATH** : Sauf pour le développement temporaire

✅ **Implémenter la détection automatique** : Votre application doit trouver les bibliothèques elle-même

✅ **Documenter les dépendances** : Créer des scripts d'installation clairs

✅ **Tester sur différentes distributions** : Les chemins peuvent varier

En suivant ces principes et en utilisant les techniques présentées dans ce document, vous serez capable de créer des applications FreePascal/Lazarus qui fonctionnent de manière fiable sur diverses distributions Linux, tout en gérant correctement les connexions aux bases de données.

⏭️ [ZEOS DBO pour accès universel](/08-bases-donnees-orm-multiplatefomes/04-zeos-dbo-acces-universel.md)
