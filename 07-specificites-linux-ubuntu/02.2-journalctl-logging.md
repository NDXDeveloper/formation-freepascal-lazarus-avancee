...
## Introduction : Les logs, vos yeux dans le système

Quand votre application FreePascal tourne comme un service systemd, elle devient invisible - pas de console, pas de fenêtre. Comment savoir ce qui se passe ? Comment déboguer les problèmes ? La réponse : **journald** et son outil **journalctl**. C'est le système de logging centralisé de systemd, et il va devenir votre meilleur ami pour comprendre et maintenir vos services.

## Comprendre journald

### Qu'est-ce que journald ?

**journald** est le démon de journalisation de systemd. Il collecte et stocke les logs de :
- Tous les services systemd
- Le noyau Linux
- Les applications qui écrivent vers syslog
...
|-------------------|----------|
| Fichiers texte éparpillés (`/var/log/*.log`) | Base de données centralisée |
| Format variable selon l'application | Format structuré uniforme |
| Rotation manuelle nécessaire | Rotation automatique |
| Recherche avec grep | Requêtes puissantes avec filtres |
| Pas de métadonnées | Métadonnées riches (PID, UID, etc.) |
| Peuvent être perdus si disque plein | Gestion intelligente de l'espace |
...
### Méthode 1 : Simple WriteLn (stdout/stderr)

La méthode la plus simple - tout ce que vous écrivez sur stdout/stderr est capturé :
...
begin
  ...
  LogInfo('Service arrêté');
end.
```

### Méthode 2 : Utilisation de syslog

Pour des logs plus structurés avec niveaux de priorité :
...
const
  ...
  // Priorités syslog
  LOG_EMERG    = 0;  // Système inutilisable
  LOG_ALERT    = 1;  // Action immédiate requise
...
begin
  ...
  LogMessage(LOG_INFO, 'Service démarré');
  LogMessage(LOG_DEBUG, 'Configuration chargée');

  try
    // Traitement
    LogMessage(LOG_INFO, 'Traitement des données');
  except
    on E: Exception do
      LogMessage(LOG_ERR, 'Erreur: ' + E.Message);
  end;

  LogMessage(LOG_INFO, 'Service arrêté');
  CloseLogging;
end.
```

### Méthode 3 : Journald natif via SD-JOURNAL

Pour une intégration maximale avec systemd :
...
begin
  ...
  // Log simple avec priorité
  LogToJournal(6, 'Service démarré'); // LOG_INFO = 6
...
### Méthode 4 : Classe de logging complète

Voici une classe réutilisable pour vos projets :
...
# Seulement les messages (sans métadonnées)
...
# Limites de taille
SystemMaxUse=1G            # Espace max pour les logs système
SystemKeepFree=100M        # Espace libre minimum
SystemMaxFileSize=100M     # Taille max d'un fichier
RuntimeMaxUse=100M         # Espace max en mémoire
...
# Forwarding
ForwardToSyslog=no         # Transférer vers syslog
ForwardToKMsg=no           # Transférer vers console kernel
ForwardToConsole=no        # Transférer vers console
...
# Niveau de log
MaxLevelStore=debug        # Niveau max stocké
...
Exposer des métriques depuis les logs :
...
begin
  ...
  try
    ...
    // Écrire dans un fichier pour node_exporter
...
### 3. Logs structurés pour parsing

```pascal
procedure LogStructured;
begin
  // Format clé=valeur pour parsing facile
...
begin
  ...
  // Masquer les numéros de carte et emails avec TRegExpr (unité RegExpr)
...
### Classe complète de sanitisation des logs
...
begin
  ...
  // Patterns par défaut pour données sensibles communes
...
begin
  ...
  RegEx := TRegExpr.Create;
  try
    // Garder 2 premiers caractères et domaine
...
begin
  ...
  RegEx := TRegExpr.Create;
  try
    // Clés API typiques (hexadécimal, base64, UUID)
...
begin
  ...
  // Si aucun type spécifié, appliquer tous
  if DataTypes = [] then
...
## Débogage avancé avec journalctl

### Analyse des crashes et core dumps

```bash
# Voir les crashes système
journalctl -p err -b

# Analyser un crash spécifique
coredumpctl list
...
### Tracer les problèmes de démarrage
...
# Voir les messages de démarrage détaillés
...
## Monitoring et alertes basés sur les logs
...
begin
  ...
  try
    ...
    // Envoyer à Zabbix via TFPHTTPClient
...
```bash
# Vérifier si le service écrit des logs
systemctl status monservice
ps aux | grep monservice

# Vérifier la configuration de journald
journalctl --verify
...
- [ ] Gestion des erreurs loggées
- [ ] IDs de corrélation pour tracer les requêtes

### ✅ Déploiement

- [ ] Configuration journald adaptée
...
Maîtriser journalctl et le logging est essentiel pour maintenir vos services FreePascal/Lazarus en production. Avec les techniques présentées, vous pouvez :

- Capturer efficacement les événements de vos applications
- Déboguer rapidement les problèmes
- Monitorer la santé de vos services
- Respecter la sécurité et la vie privée
- Intégrer vos logs avec des outils d'analyse modernes

Les logs sont votre fenêtre sur le comportement de vos applications en production. Investissez du temps pour bien les implémenter, vous vous remercierez lors du prochain incident à résoudre !

Prochaine étape : Apprendre à interagir avec D-Bus pour la communication inter-processus sous Linux.

⏭️ [D-Bus et communication inter-processus](/07-specificites-linux-ubuntu/03-dbus-communication-inter-processus.md)
