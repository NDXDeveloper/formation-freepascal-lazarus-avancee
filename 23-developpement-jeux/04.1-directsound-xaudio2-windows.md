...
L'audio est un élément crucial pour l'immersion dans les jeux. Sur Windows, Microsoft fournit deux API principales pour gérer le son : **DirectSound** (ancienne génération) et **XAudio2** (moderne). Ces technologies font partie de DirectX et offrent des fonctionnalités avancées pour jouer des sons, gérer le volume, appliquer des effets et créer du son spatial 3D.

### Qu'est-ce que DirectSound ?

**DirectSound** est l'API audio historique de DirectX, introduite en 1995. Bien qu'elle soit considérée comme obsolète depuis Windows Vista, elle reste largement supportée et simple à utiliser pour des besoins audio basiques.

**Avantages** :
- Simple et bien documentée
- Largement compatible (Windows XP à Windows 11)
- Suffisante pour la plupart des jeux 2D
- Bonne pour l'apprentissage

**Inconvénients** :
- Performance limitée
- Pas de support matériel moderne
- API vieillissante
- Latence plus élevée que XAudio2

### Qu'est-ce que XAudio2 ?

**XAudio2** est l'API audio moderne de Microsoft, introduite avec DirectX 10. Elle remplace DirectSound et offre de meilleures performances, une latence plus faible et plus de flexibilité.
...
- Nécessite Windows 7 minimum (Windows 10 pour les dernières versions)
- Documentation parfois technique

### Quand utiliser quoi ?

**Utilisez DirectSound si** :
- Vous débutez dans la programmation audio
- Vous créez un jeu 2D simple
- Vous devez supporter Windows XP
- Vous avez besoin d'une solution simple et rapide

**Utilisez XAudio2 si** :
- Vous créez un jeu 3D
...
DirectSound est inclus dans Windows, aucune installation supplémentaire n'est nécessaire. Les fichiers nécessaires :
- `dsound.dll` - Bibliothèque DirectSound (présente sur tous les Windows)

### Créer les bindings Pascal

DirectSound est une API COM (Component Object Model). Nous devons créer des bindings Pascal pour l'utiliser.
...
const
  ...
  // Volume (échelle logarithmique en centièmes de décibels)
  DSBVOLUME_MIN = -10000;
  DSBVOLUME_MAX = 0;
...
type
  ...
  TWaveFormatEx = packed record
    ...
    nChannels: Word;            // Nombre de canaux (1 = mono, 2 = stéréo)
    nSamplesPerSec: DWORD;      // Fréquence d'échantillonnage (ex: 44100 Hz)
    nAvgBytesPerSec: DWORD;     // Débit moyen
    nBlockAlign: Word;          // Alignement des blocs
    wBitsPerSample: Word;       // Bits par échantillon (8, 16, etc.)
    cbSize: Word;               // Taille des données supplémentaires
  end;
  ...
  TDSBufferDesc = packed record
    ...
    dwFlags: DWORD;             // Flags de capacités
    dwBufferBytes: DWORD;       // Taille du buffer en octets
    dwReserved: DWORD;          // Réservé
...
// Fonction pour créer un objet DirectSound
...
Créons une classe Pascal plus facile à utiliser :
...
type
  ...
  public
    ...
    procedure SetMasterVolume(Volume: Single); // 0.0 à 1.0
...
begin
  ...
  try
    ...
    // Vérifier le format (doit être "RIFF" et "WAVE")
    ...
    if Failed(DS.CreateSoundBuffer(BufferDesc, FBuffer, nil)) then
      raise Exception.Create('Impossible de créer le buffer DirectSound');

    // Verrouiller le buffer pour y écrire les données
    ...
    FPan := 0; // Centré
    FBuffer.GetFrequency(FFrequency); // Fréquence originale
...
begin
  ...
  if Failed(DirectSoundCreate(nil, FDirectSound, nil)) then
    raise Exception.Create('Impossible de créer DirectSound');

  // Définir le niveau de coopération
  if Failed(FDirectSound.SetCooperativeLevel(WindowHandle, DSSCL_PRIORITY)) then
    raise Exception.Create('Impossible de définir le niveau de coopération');
end;
...
var
  ...
  VolumeDB: Integer;
begin
  // Convertir 0.0-1.0 en décibels (-10000 à 0)
  if Volume <= 0 then
...
begin
  ...
  try
    ...
    WriteLn('Appuyez sur J pour sauter, C pour pièce, Q pour quitter');
...
begin
  ...
  // Volume : de 0.0 (silence) à 1.0 (maximum)
  Volume := 0.5; // 50%
  Sound.Volume := Round(2000 * Log10(Volume));

  // Pan : de -10000 (gauche) à 10000 (droite), 0 = centré
  Pan := -5000; // Légèrement à gauche
  Sound.Pan := Pan;
...
begin
  ...
  // Obtenir la fréquence originale
  OriginalFreq := Sound.Frequency;

  // Jouer à la fréquence normale
  Sound.Play;
  Sleep(2000);
...
begin
  ...
  // Mettre à jour tous les sons de cette catégorie
  for i := 0 to FSounds.Count - 1 do
  begin
...
### Pool de sons pour effets fréquents
...
begin
  // Créer un pool de 10 instances du son de tir
...
### Introduction à XAudio2

**XAudio2** est l'API audio moderne de Microsoft, offrant des performances supérieures et une latence minimale. Elle est particulièrement adaptée aux jeux modernes nécessitant un grand nombre de sons simultanés et des effets audio avancés.
...
Pour une compatibilité maximale, nous utiliserons XAudio2.9, qui est disponible sur tous les systèmes Windows modernes.

### Installation et prérequis

XAudio2.9 est intégré à Windows 10/11, aucune installation supplémentaire n'est nécessaire. Les fichiers nécessaires :
- `xaudio2_9.dll` - Bibliothèque XAudio2 (présente sur Windows 10+)

### Créer les bindings Pascal pour XAudio2
...
type
  // Callback simple pour les événements
...
begin
  ...
  try
    ...
    // Allouer et lire les données audio
...
begin
  ...
  // Arrêter et vider les buffers existants
  FSourceVoice.Stop;
...
begin
  ...
  // XAudio2 utilise un ratio de fréquence (0.5 = moitié, 2.0 = double)
  if Assigned(FSourceVoice) then
...
begin
  ...
  if Failed(XAudio2Create(FXAudio2, 0, 1)) then
    raise Exception.Create('Impossible de créer XAudio2');

  // Créer la mastering voice
  if Failed(FXAudio2.CreateMasteringVoice(FMasterVoice, 2, 44100, 0, nil, nil, 0)) then
    raise Exception.Create('Impossible de créer la mastering voice');
...
## Fonctionnalités avancées XAudio2
...
begin
  ...
  // Simplifié : basé sur la position X relative
  Pan := DX / (Distance + 0.001);

  // Appliquer le volume
  Volume := Attenuation;

  // Effet Doppler simplifié (basé sur la vélocité)
  // TODO: Implémenter calcul Doppler complet
end;
...
Pour les gros fichiers (musique de fond), le streaming évite de charger tout le fichier en mémoire.
...
begin
  ...
  FFile.Position := 44; // Après l'en-tête WAV
...
begin
  // Un buffer est terminé, remplir le suivant
  FillNextBuffer;
end;
...
begin
  ...
  begin
    StreamingMusic.Update; // IMPORTANT : appeler régulièrement
...
La réverbération simule l'écho naturel dans différents environnements.
...
begin
  // Appliquer les paramètres à la source voice
  // Note: Nécessite la création d'un effect chain, simplifié ici
...
Utile pour simuler les sons étouffés (derrière un mur, sous l'eau, etc.)
...
begin
  ...
  FilterParams.Frequency := CutoffFrequency; // 0.0 - 1.0 (normalisé)
  FilterParams.OneOverQ := 1.0; // Qualité du filtre
...
begin
  // Fréquence de coupure basse pour effet sous l'eau
...
Utile pour les voix radio, téléphones, etc.
...
begin
  ...
  // Ajouter de la distorsion (simplifié)
  Sound.Volume := 0.9;
end;
```
...
Les **submix voices** permettent de grouper plusieurs sons et d'appliquer des effets à l'ensemble.
...
| Caractéristique | DirectSound | XAudio2 |
|----------------|-------------|---------|
| **Performance** | Moyenne | Excellente |
| **Latence** | 20-50 ms | 5-20 ms |
| **Complexité** | Simple | Moyenne |
| **Effets intégrés** | Basiques | Avancés |
| **Audio 3D** | Oui (ancien) | Oui (moderne) |
| **Multi-threading** | Limité | Excellent |
| **Windows XP** | Oui | Non |
| **Windows 10+** | Oui | Oui (natif) |
| **Streaming** | Possible | Optimisé |
| **Nombre max de sons** | ~32 | Illimité (selon CPU) |
...
- Vous développez un jeu moderne (2020+)
- Vous utilisez beaucoup de sons simultanés
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          // Tenter de parser une catégorie
          for Cat := Low(TAudioCategory) to High(TAudioCategory) do
          begin
...
begin
  ...
  try
    ...
    begin
      ...
      // Mettre à jour l'audio (fondus, etc.)
    ...
    // Sauvegarder les paramètres audio avant de quitter
...
### 1. Limiter le nombre de sons simultanés
...
begin
  ...
  // Chercher le son de priorité la plus basse
  ...
  // Si le nouveau son est plus prioritaire, arrêter le moins prioritaire
...
begin
  // Charger le son dans un thread séparé
...
begin
  ...
  // Continuer le chargement pendant que l'audio se charge en arrière-plan
  LoadLevelGeometry;
  LoadLevelTextures;
end;
```
...
var
  ...
  Sound: TManagedSound;
begin
  // Supprimer les sons les moins récemment utilisés
...
begin
  ...
  case Ext of
    ...
    begin
      ...
      raise Exception.Create('Format audio non supporté: ' + Ext);
  end;
end;
```
...
begin
  // Appliquer les paramètres depuis les contrôles UI
...
begin
  case Error of
    XAUDIO2_E_INVALID_CALL:
      ErrorMsg := 'Appel invalide à XAudio2';
    XAUDIO2_E_DEVICE_INVALIDATED:
      ErrorMsg := 'Périphérique audio invalidé (débranché?)';
    E_OUTOFMEMORY:
      ErrorMsg := 'Mémoire insuffisante pour l''audio';
    else
...
begin
  ...
  begin
    WriteLn('Aucun périphérique audio détecté');
    FDummyMode := True;
  end;
end;
```
...
begin
  ...
  // CPU usage nécessiterait un profiler plus sophistiqué
  Result.CPUUsage := 0.0;
...
**Pour la musique** :
- OGG Vorbis (compression avec perte, bonne qualité)
- Taux d'échantillonnage : 44100 Hz
- Bitrate : 128-192 kbps

**Pour les effets sonores** :
- WAV (non compressé, latence minimale)
- Taux d'échantillonnage : 22050 ou 44100 Hz
- 16 bits, mono pour les petits sons

**Pour les voix** :
- WAV ou OGG selon la longueur
- 44100 Hz, 16 bits, mono ou stéréo
...
const
  ...
  PRIORITY_LOW = 25;        // Sons ambiants, détails
  PRIORITY_MINIMAL = 10;    // Peut être ignoré si trop de sons
```

### 5. Volumes par défaut
...
## Checklist de développement audio

### Avant de commencer

- [ ] Choisir entre DirectSound et XAudio2
- [ ] Définir les catégories audio nécessaires
- [ ] Planifier l'organisation des fichiers
- [ ] Établir les conventions de nommage
- [ ] Décider des formats audio à utiliser

### Pendant le développement

- [ ] Implémenter le chargement de base
- [ ] Ajouter le contrôle du volume
- [ ] Implémenter les fondus enchaînés
- [ ] Gérer les priorités des sons
...
- [ ] Optimiser la mémoire audio
- [ ] Tester sans périphérique audio
- [ ] Vérifier la latence
...
Le développement audio avec DirectSound et XAudio2 sur Windows offre un contrôle complet et des performances excellentes pour les jeux FreePascal/Lazarus.

### Points clés à retenir

✅ **DirectSound** : Simple et compatible, idéal pour débuter  
✅ **XAudio2** : Moderne et performant, pour les jeux avancés  
✅ **Gestion des catégories** : Organiser les sons par type  
✅ **Priorités** : Gérer intelligemment les sons simultanés  
✅ **Streaming** : Pour les fichiers volumineux (musique)  
✅ **Effets** : Reverb, filtres pour l'immersion  
✅ **Options utilisateur** : Menu complet de contrôle audio  
✅ **Performance** : Optimiser mémoire et CPU  
✅ **Gestion d'erreurs** : Mode de secours sans audio

### L'audio fait le jeu

Un bon système audio peut transformer un jeu correct en une expérience mémorable. Avec FreePascal et les API Windows, vous avez tous les outils nécessaires pour créer une ambiance sonore professionnelle.

N'oubliez pas : testez votre audio sur différents matériels, offrez des options de personnalisation aux joueurs, et surtout, assurez-vous que vos sons contribuent au gameplay plutôt que de le distraire.

Bon développement audio ! 🎵🎮


⏭️ [ALSA/PulseAudio (Linux)](/23-developpement-jeux/04.2-alsa-pulseaudio-linux.md)
